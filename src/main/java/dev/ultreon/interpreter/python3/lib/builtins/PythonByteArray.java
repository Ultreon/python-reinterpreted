...
public class PythonByteArray extends PythonBuiltin implements Convertible {
    private byte[] bytes;
    private boolean init = true;

    public PythonByteArray(Context context, byte[] bytes) {
        super(((PythonStdLib)context.getStdLib()).getBuiltins().getBytesClass());
        this.bytes = bytes;
    }

    public PythonByteArray(Context context, int length) {
        super(((PythonStdLib)context.getStdLib()).getBuiltins().getBytesClass());
        this.bytes = new byte[length];
    }

    public PythonByteArray(Context context) {
        super(((PythonStdLib)context.getStdLib()).getBuiltins().getBytesClass());
        this.bytes = new byte[0];
    }


    public PythonByteArray(PythonClass theClass) {
        super(theClass);
        init = false;
    }
    ...
    public void initialize(InterpInitializerList visit, FieldInitializer fieldInitializer) {
        ...
        if (args.length != 0) {
            if (args.length == 1) {
                Value arg = args[0];
                if (arg.ptr instanceof PythonByteArray) {
                    bytes = ((PythonByteArray) arg.ptr).bytes;
